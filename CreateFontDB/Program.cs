...
namespace CreateFontDB {
	static class Program {
		static void Main( string[] args ) {
			File.Delete("./font.db");
			using( var conn = new SQLiteConnection( "Data Source=" + "./font.db" ) ) {
				conn.Open();
				using( var command = conn.CreateCommand() ) {
					command.CommandText = "CREATE TABLE fonts(char TEXT unique,fontdata TEXT PRIMARY KEY);";
					command.ExecuteNonQuery();
					foreach( var letter in LETTER_LIST ) {
						string fontdata;
						if( (fontdata = ReplaceFontData( letter )) != null ) {
						} else {
							var bmp = CreateBmp( letter );
							fontdata = "";
