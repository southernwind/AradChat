...
namespace AradChat {
	public class Hc {
		...
		public async Task<string> Navigate( string url, MultipartFormDataContent multipart ) {
			this._hc.DefaultRequestHeaders.Host = Url2Host( url );

			HttpResponseMessage hrm;
			for( var i = 0; ; i++ ) {
				try {
					hrm = await this._hc.PostAsync( url, multipart );
					break;
				} catch {
					Thread.Sleep( this.waitTime );
					if( i > this.retryCount ) {
		...
		public async Task<Bitmap> LoadImageFromUrl( string url ) {
			...
			for( var i = 0; ; i++ ) {
				try {
					byteArray = await this._hc.GetByteArrayAsync( url );
					break;
				} catch {
					Thread.Sleep( this.waitTime );
					if( i > this.retryCount ) {
			...
			return (Bitmap)imgconv.ConvertFrom( byteArray );
		}
		public async Task<byte[]> LoadBinaryFromUrl( string url ) {
			this._hc.DefaultRequestHeaders.Host = Url2Host( url );

			for( var i = 0; ; i++ ) {
				try {
					return await this._hc.GetByteArrayAsync( url );
				} catch {
					Thread.Sleep( this.waitTime );
					if( i > this.retryCount ) {
						return null;
	...
	public static class StringExtensions {
		public static string Cleanbr( this string str ) {
			return Regex.Replace( ( str ).Replace( Environment.NewLine, "" ), "\r|\n|\t", "" );
		}
	}
}
